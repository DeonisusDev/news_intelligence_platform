...
_CARD_COLUMNS = "cluster_id, summary, topic, sentiment, sentiment_score, keywords, article_count, enriched_at"
...
def _row_to_card(row: tuple) -> SummaryCard:
    return SummaryCard(
        cluster_id=row[0], summary=row[1], topic=row[2], sentiment=row[3],
        sentiment_score=row[4], keywords=row[5], article_count=row[6], enriched_at=row[7],
    )
...
def get_summary_detail(
    cluster_id: str, client: Client = Depends(get_clickhouse_client),
) -> SummaryDetail:
    ...
    sources = [
        SourceArticle(
            source_name=r[0], source_provider=r[1], title=r[2], url=r[3],
            url_to_image=r[4], published_at=r[5], category=r[6],
        )
